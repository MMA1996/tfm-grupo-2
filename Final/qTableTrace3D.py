...
from deliveryMap3D import Status
...
class QTableTraceModel(AbstractModel):
    ...
    def train(self, stop_at_convergence=False, **kwargs):
        ...
        while True:
            ...
            if not start_list:
                start_list = self.environment.allowableCells.copy()
            start_cell = random.choice(start_list)
            start_list.remove(start_cell)

            cell, height = self.environment.reset(start_cell)
            cell = tuple(cell.flatten())  # change np.ndarray to tuple so it can be used as dictionary key
            ...
            while True:
                if np.random.random() < exploration_rate:
                    ...
                    action = self.predict(cell, height)

                try:
                    etrace[(cell, height, action)] += 1
                except KeyError:
                    etrace[(cell, height, action)] = 1

                next_cell, next_height, reward, status = self.environment.step(action)
                next_cell = tuple(next_cell.flatten())

                cumulative_reward += reward

                if (cell, height, action) not in self.Q.keys():  # ensure value exists for (state, action) to avoid a KeyError
                    self.Q[(cell, height, action)] = 0.0

                max_next_Q = max([self.Q.get((next_cell, next_height, a), 0.0) for a in self.environment.actions])

                # update Q's in trace
                delta = reward + discount * max_next_Q - self.Q[(cell, height, action)]
            ...
            logging.info("episode: {:d} | status: {:4s} | e: {:.5f}"
                         .format(episode, status.name, exploration_rate))
            ...
            if episode % check_convergence_every == 0:
                ...
                w_all, win_rate, previousWin = self.environment.checkWinAll(self, previousWin)
                win_history.append((episode, win_rate))
                if (w_all is True) and (stop_at_convergence is True) and (previousWin > 2):
    ...
    def q(self, cell, height):
        """ Get q values for all actions for a certain state. """
        if type(cell) == np.ndarray:
            cell = tuple(cell.flatten())

        return np.array([self.Q.get((cell, height, action), 0.0) for action in self.environment.actions])

    def predict(self, cell, height):
        """ Policy: choose the action with the highest value from the Q-table.
            Random choice if multiple actions have the same (max) value.
            :param np.ndarray state: game state
            :return int: selected action
        """
        q = self.q(cell, height)
