...
from deliveryMap import Status
...
class QTableModel(AbstractModel):
    ...
    def train(self, stop_at_convergence=False, **kwargs):
        """ Train the model.
            ...
            :keyword float explorationRate: (epsilon) 0 = preference for exploring (0 = not at all, 1 = only)
            :keyword float explorationDecay: exploration rate reduction after each random step (<= 1, 1 = no at all)
            :keyword float learningRate: (alpha) preference for using new knowledge (0 = not at all, 1 = only)
            :keyword int episodes: number of training games to play
            :return int, datetime: number of training episodes, total time spent
        """
        discount = kwargs.get("discount", 0.90)
        explorationRate = kwargs.get("explorationRate", 0.10)
        explorationDecay = kwargs.get("explorationDecay", 0.995)  # % reduction per step = 100 - exploration decay
        learningRate = kwargs.get("learningRate", 0.10)
        ...
        # for episode in range(1, episodes + 1):
            ...
            if not deliveryList:
                deliveryList = self.environment.empty.copy()
            deliveryCell = random.choice(deliveryList)
            deliveryList.remove(deliveryCell)

            state = self.environment.reset(deliveryCell)
            ...
            while True:
                ...
                self.Q[(state, action)] += learningRate * (reward + discount * max_next_Q - self.Q[(state, action)])
            ...
            logging.info("episode: {:d} | status: {:4s} | e: {:.5f}"
                         .format(episode, status.name, explorationRate))
            ...
            if episode % check_convergence_every == 0:
                ...
                w_all, win_rate, cellsRanking, sumForProb = self.environment.checkWinAll(self)
                win_history.append((episode, win_rate))
                if w_all is True and stop_at_convergence is True:
                    logging.info("won from all start cells, stop learning")
                    break                           

            explorationRate *= explorationDecay  # explore less as training progresses
            episode += 1
